export function makeNPC3(k, player, x, y, onKeyReceived) {
  let canTalk = false;
  let dialogIndex = 0;
  let isTalking = false;
  ...
  const npc3 = k.add([
    ...
    k.sprite("werebear"),
    k.anchor("center"),
    k.area({ shape: new k.Rect(k.vec2(0, 0), 20, 10) }),
    k.scale(2),
    "npc3"
  ]);
  ...
  // --- 2. ตรวจสอบระยะห่าง ---
  npc3.onUpdate(() => {
    const dist = player.pos.dist(npc3.pos);
    if (dist < 60) {
      canTalk = true;
    } else {
      canTalk = false;
